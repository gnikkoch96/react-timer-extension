...
export default function App() {
  ...
  const [newSound, setNewSound] = useState("chime");

  // Load presets and active timer status
  useEffect(() => {
    chrome.storage.local.get(
      ["timerPresets", "timerEndTime", "timerLabel"],
      (result) => {
    ...
    <div style={{ padding: "16px", width: "260px", fontFamily: "sans-serif" }}>
      ...
      {timeLeft !== null ? (
        <div style={{ textAlign: "center", marginBottom: "16px" }}>
          ...
          <div
            style={{ fontSize: "32px", fontWeight: "bold", margin: "4px 0" }}
          >
            {formatTime(timeLeft)}
          </div>
          <button
      ...
      <div
        style={{ maxHeight: "140px", overflowY: "auto", marginBottom: "12px" }}
      >
        {presets.map((preset) => (
          <div
            key={preset.id}
      ...
      >
        ...
        <div style={{ display: "flex", gap: "6px" }}>
          ...
          >
            <option value="chime">Chime (.ogg)</option>
            <option value="digital">Digital (.ogg)</option>
            <option value="bell">Bell (.ogg)</option>
          </select>
        </div>
      </form>
    </div>
